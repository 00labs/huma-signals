...
from huma_signals.adapters.request_network.adapter import RequestNetworkInvoiceAdapter
from huma_signals.adapters.request_network.models import Invoice


def describe_adapter():
    def describe_get_payments():
        ...
        @pytest.fixture(scope="session", autouse=True)
        def to_address() -> str:
            return "0x63d6287d5b853ccfedba1247fbeb9a40512f709a".lower()  # gitleaks:allow

        def it_returns_payer_payment_history(rn_subgraph_endpoint_url, from_address):
            payments = RequestNetworkInvoiceAdapter._get_payments(from_address, None, rn_subgraph_endpoint_url)
            assert len(payments) > 0
            assert payments[-1]["from"] == from_address
            assert payments[-1]["to"].startswith("0x")

        def it_returns_payee_payment_history(rn_subgraph_endpoint_url, to_address):
            payments = RequestNetworkInvoiceAdapter._get_payments(None, to_address, rn_subgraph_endpoint_url)
            assert len(payments) > 0
            assert payments[-1]["to"] == to_address
            assert payments[-1]["from"].startswith("0x")

        def it_returns_pair_payment_history(rn_subgraph_endpoint_url, from_address, to_address):
            payments = RequestNetworkInvoiceAdapter._get_payments(from_address, to_address, rn_subgraph_endpoint_url)
            assert len(payments) > 0
            assert payments[-1]["to"] == to_address
            assert payments[-1]["from"] == from_address
    ...
    def describe_fetch():
        ...
        def it_can_fetch_signals(
            ...
            signals = RequestNetworkInvoiceAdapter.fetch(
                chain_name="goerli",
            ...
            assert signals.payer_total_amount == Decimal("0")
            assert signals.payee_tenure == 0
            assert signals.payee_recent == 999
            assert signals.payee_count == 0
            assert signals.payee_total_amount == Decimal("0")
            assert signals.mutual_count == 0
            assert signals.mutual_total_amount == Decimal("0")
        ...
        @mock.patch(
            ...
            return_value=Invoice(
                ...
                currency="USDC",
                status="UNPAID",
                creation_date=datetime(2022, 12, 25),
                due_date=datetime(2023, 1, 25),
            ),
        )
        def it_can_calculate_signals_with_mocked_invoice(
            ...
            assert signals.payer_total_amount > Decimal("100_000_000")
            assert signals.payee_tenure > 600
            assert signals.payee_recent < 999
            assert signals.payee_count > 30
            assert signals.payee_total_amount >= Decimal("6_000_000")
            assert signals.mutual_count > 35
            assert signals.mutual_total_amount >= Decimal("6_000_000")
